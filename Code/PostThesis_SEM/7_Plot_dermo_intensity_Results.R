# Header -----------------------------------------------------------------------
# 
# Plot results for models that use dermo intensity
# 
# Plot the results of the best model(s) for interpretation.
# Plot the 2 temporal/2 spatial group model results only (would be selected as
# the "optimal model)
# 
...
if(write_files == T){
    # set paths
    write_derived_dat_path <- "./Derived_Data/PostThesis_SEM/7_Plot_dermo_intensity_Results"
    fig_gen_path <- "./Figures/PostThesis_SEM"
    write_fig_path <- paste0(fig_gen_path, "/7_Plot_dermo_intensity_Results")
    # create folders
...
fac_lev_rel <- c(fac_lev_rel, paste0("Mean_Intensity~", c("temp_s_cen", "sal_cen")))
fac_lev_rel <- c(fac_lev_rel, "M_Med_A_Bar_Per~Mean_Intensity")
...
#     'Mean_Intensity~temp_s_cen' = "Temperature -> Dermo Mean Intensity",
#     'Mean_Intensity~sal' = "salinity -> Dermo Mean Intensity",
#     'Dermo_Med_Annual_Per~MSX_Prev_Per'="Dermo Mean Intensity -> % Mortality"
# )
...
fac_rel_labels <- c("Temperature -> % MSX Prevalence",
    ...
    "Temperature -> Dermo Mean Intensity",
    "Salinity -> Dermo Mean Intensity",
    "Dermo Mean Intensity -> % Mortality"
)
...
ggplot(ts_grp, aes(x = sal_zone, y = est, fill = years)) +
    ...
    facet_wrap(~fac_relationship, scales ="free",ncol = 3)+#, labeller = as_labeller(fac_rel_labels))+
...
lhs_fac <- c("MSX_Prev_Per", "Mean_Intensity", "M_Med_A_Bar_Per")
...
#general dermo gg
dermo_M_gen_biplot <- ggplot(dat, aes(x = Mean_Intensity, y = M_Med_A_Bar_Per))+
    geom_smooth(se = F)+
    geom_point()+
    xlab("Dermo Mean Intensity")+
    #xlim(-10,110)+
...
ggsave(paste0(write_fig_path, "/biplot_ts_2_spat_2_M_v_Dermo_Int.png"), 
...
ggsave(paste0(write_fig_path, "/biplot_ts_2_spat_3_M_v_Dermo_Int.png"), 
    device = "png", width = 6.5*2, height = 3.5*2)
...
dermo_MSX_M_plot <- plot_ly(dat, 
    ...
    layout(scene = list(
        ...
        yaxis = list(title = "Dermo Intensity"),
